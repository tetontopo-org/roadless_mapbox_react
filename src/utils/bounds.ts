...
export async function getTileJSONBounds(tileset: string, token: string) {
  try {
    ...
    return { bounds: tj.bounds as [number, number, number, number] | null, center: tj.center as number[] | null }
  } catch (err) {
...
export function unionBounds(a: [number, number, number, number] | null | undefined,
                            b: [number, number, number, number] | null | undefined) {
  if (!a) return b || null
  if (!b) return a
  const [aw, as, ae, an] = a
  const [bw, bs, be, bn] = b
  return [Math.min(aw,bw), Math.min(as,bs), Math.max(ae,be), Math.max(an,bn)] as [number, number, number, number]
}

export function fitBounds(map: mapboxgl.Map, b: [number, number, number, number]) {
  const [w,s,e,n] = b
